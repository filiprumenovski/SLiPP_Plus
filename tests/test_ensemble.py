...
def _toy_ten_class_row(
    iteration: int, row_index: int, model: str, probs: dict[str, float], y_true: int
) -> dict:
    d = {
        "iteration": iteration,
        "model": model,
...
def test_average_softprobs_is_mean_of_models() -> None:
    ...
    rows: list[dict] = []
    rows.append(_toy_ten_class_row(0, 0, "rf", {"CLR": 0.6, "STE": 0.2, "PP": 0.2}, y_true=3))
    rows.append(_toy_ten_class_row(0, 0, "xgb", {"CLR": 0.2, "STE": 0.6, "PP": 0.2}, y_true=3))
    rows.append(_toy_ten_class_row(0, 1, "rf", {"PP": 1.0}, y_true=8))
    rows.append(_toy_ten_class_row(0, 1, "xgb", {"PP": 1.0}, y_true=8))
...
def test_score_summary_has_required_keys() -> None:
    ...
    for ri in range(4):
        ...
        probs[true_c] = 1.0
        for model in ("rf", "xgb", "lgbm"):
            rows.append(_toy_ten_class_row(0, ri, model, probs, y_true=CLASS_10.index(true_c)))
    df = pl.DataFrame(rows)
    ens = average_softprobs(df)
    s = score_summary(ens)
    for key in (
