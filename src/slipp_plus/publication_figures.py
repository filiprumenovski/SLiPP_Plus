...
# ----------------------------------------------------------------------
# Data coverage gap (BioDolphin vs Chou)
# ----------------------------------------------------------------------


def figure_data_coverage_gap(
    *,
    out_dir: Path,
    stem: str = "figure_data_coverage_gap",
    formats: Sequence[str] = DEFAULT_FORMATS,
    chou_pdb_count: int = 780,
    chou_pocket_count: int = 1981,
    chou_lipid_class_count: int = 5,
    chou_cutoff_label: str = "PDB snapshot 27 Apr 2023",
    biodolphin_pdb_count: int = 14_891,
    biodolphin_entry_count: int = 127_359,
    biodolphin_unique_lipids: int = 2_619,
    biodolphin_cutoff_label: str = "BioLiP2 + PDB through 6 Sep 2024",
    title: str = "Data availability gap — public lipid-pocket coverage we have not yet exploited",
    subtitle: str = "Chou et al. 2024 used PDB-only with a ≥20-entries-per-lipid filter; BioDolphin (Lin et al., Nat Comm Chem 2024) covers ~19× more PDB structures.",
) -> dict[str, Path]:
    """Bar chart contrasting the Chou et al. dataset with BioDolphin coverage."""

    apply_publication_style()
    fig, axes = plt.subplots(1, 3, figsize=(13.5, 5.6))

    palette = ["#9AA0A6", "#16A085"]

    def _bars(ax: plt.Axes, label: str, values: list[int], unit: str) -> None:
        ...
        ax.set_xticklabels(["Chou et al. 2024", "BioDolphin v1.1"], fontsize=9.5)
        ax.set_ylabel(unit)
    ...
    _bars(
        ...
        [chou_pdb_count, biodolphin_pdb_count],
        "PDB structures",
    )
    _bars(
        axes[1],
        "Pocket / interaction entries",
        [chou_pocket_count, biodolphin_entry_count],
        "pocket-equivalent rows",
    )
    _bars(
        axes[2],
        "Distinct lipid molecules",
        [chou_lipid_class_count, biodolphin_unique_lipids],
        "lipid molecules",
    )
    ...
    fig.text(
        0.05,
        0.022,
        f"Sources: Chou et al. ({chou_cutoff_label}). BioDolphin v1.1 ({biodolphin_cutoff_label}).",
        ha="left",
        fontsize=8.5,
        color="#666666",
        style="italic",
...
# ----------------------------------------------------------------------
# Pipeline schematic — Fig 7's spiritual successor
# ----------------------------------------------------------------------


def figure_pipeline_schematic(
    *,
    out_dir: Path,
    stem: str = "figure_pipeline_schematic",
    formats: Sequence[str] = DEFAULT_FORMATS,
    title: str = "SLiPP++ — from binary lipid prediction to a holdout-disciplined ten-class softmax",
    subtitle: str = "Six stages: data → features → flat ensemble → hierarchical specialists → multi-objective HPO → external validation.",
) -> dict[str, Path]:
    """Six-stage pipeline schematic with annotated arrows.

    No dataset inputs — this is pure layout.
    """
    ...
    stages = [
        ("DATA", "training_pockets.csv\n15,219 pockets, 10 classes\n(Chou et al. 2024)", "#34495E"),
        ("FEATURES", "paper17 + aa20 + shell12\n+ sterol_chemistry + tunnel_shape\n+ derived ≈ 80–104 columns", "#16A085"),
        ("FLAT ENSEMBLE", "RF + XGB + LGBM + CatBoost\nmean / stacked-LR meta-learner", "#9B59B6"),
        ("HIERARCHICAL", "lipid gate → 5-class family head\n→ STE / OLA / PLM specialists\n→ utility-gated boundary heads", "#E67E22"),
        ("HPO", "Optuna NSGA-II + Hyperband\nMulti-objective: internal × holdout\nGrouped UniProt CV", "#C0392B"),
        ("VALIDATION", "25-iter grouped CV\napo-PDB (n=117) + AlphaFold (n=149)\nblind-holdout significance gate", "#0F8B8D"),
    ]

    box_w, box_h = 14.0, 12.0
    gap = 1.5
    total = len(stages) * box_w + (len(stages) - 1) * gap
    start = (100 - total) / 2

    for i, (header, body, color) in enumerate(stages):
        x = start + i * (box_w + gap)
        y = 9
        box = FancyBboxPatch(
            (x, y),
    ...
    ax.text(
        callout_x + 1.2,
        callout_y + 1.5,
        "Replace DATA box with BioDolphin v1.1 (14,891 PDB structures, 127,359 entries, 2,619 lipids; cutoff Sep 2024) "
        "→ ~19× pocket coverage and >1,200 lipid molecules currently excluded by the ≥20-entries filter.",
        fontsize=9,
        color="#5C2E08",
    )
